...
3. 复杂的权限设计（主应用控制，进入主应用后，获取用户角色和权限列表和数据列表，过滤路由列表只渲染特定的页面和限制接口数据）
   - 应用权限
      - 页面权限
         - 由用户的角色权限决定
            - path
...
`Vue3 + Ts + Pinia + echarts + naiveui + tailwindcss`

- [x] 制定代码和git规范(`standard` 规范)、自动化版本管理
...
- [x] 批量导出
   - 服务端导出实现逻辑
      ...
      1. node 端通过 `puppeteer` 创建浏览器，访问线上小蓝书页面，`通过点击事件触发导出`。
