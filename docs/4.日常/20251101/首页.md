...
```js
/*
	1. promise.then的地狱回调，或者await
	2. for... of（for...in 不支持异步）
	3. 请求队列
*/
```
...
- 原理：Vue 是异步更新 Dom，为了拿到更新后的DOM，做的一种适配异步更新机制的任务
- 过程：
1. 当响应式数据变化时，DOM 更新的 watcher 不会立刻执行，而是会创建异步更新队列，将 Watcher 加入**异步更新队列** 并去重（避免同一数据多次更新导致重复渲染）
  2. $nextTick 的回调代码，会被添加到异步更新队列的末尾
3. Vue 等待同步代码执行完毕，执行更新队列：视图更新 => nextTick回调
...
2. 性能优化
   ...
   - `编译优化`（[提高虚拟 DOM 运行时性能](https://cn.vuejs.org/guide/extras/rendering-mechanism.html#compiler-informed-virtual-dom)）
     - 更新类型标记 **patchFlag**
     - 函数缓存 **cacheHandlers**
     - 静态节点提升 **hoistStatic**
...
4. 底层优势：`defineProperty` 只能拦截 `get`  和  `set`，而 `Proxy` 提供了多达13种拦截操作，覆盖几乎所有的基础行为，且和 `Reflect` 完全对应，拓展性强
...
1. 强缓存
   - **expires**（http 1.0）：绝对值
   - **cache-control**（http 1.1）：相对值，相关字段（max-age、no-cache、no-store）
2. 协商缓存
   - **ETag**：`If-None-Match`
...
>
> 1. Webpack 工作原理 / 流程？
> 2. loader / plugin 工作原理 ？
> 3. Vite 与 Webpack 核心差异？
> 4. 常见性能优化手段？
> 5. CommonJs 和 Esm
...
- 1.1：持久化连接、管道化（并行）
- 2.0：二进制分帧、多路复用、头部压缩（HPACK）、服务器推送
...
1. 连接性：TCP面向；UDP无连接即发送数据前无需建立连接
2. 可靠性：TCP可靠传输，数据正确且有序；UDP不可靠，数据丢失且乱序
3. 拥塞/流量控制：TCP有流量控制（滑动窗口）和拥塞控制（慢启动）；UDP没有
4. 场景：TCP适用可靠性要求高；UDP适用直播、音视频通话等
...
1. **为什么需要四次？**
   ...
   TCP 是**全双工通信**（双方可同时收发数据），**关闭连接时，每一方的发送通道和接收通道都需要单独关闭**。

   第一次 `FIN` 关闭主动方的发送通道，第三次 `FIN` 关闭被动方的发送通道。

   两次 `ACK` 分别确认，因此需要四次交互。

2. **2MSL 作用**

   MSL（**最大报文生存时间**）是报文在网络中的最长存活时间。
   
   主动关闭方等待 2MSL，一是确保自己发送的最后一个 `ACK` 能被被动方收到（若丢失，被动方会重发 `FIN`，主动方还能再次回复）；
   
   二是让网络中残留的该连接报文都失效，避免新连接被旧报文干扰。
